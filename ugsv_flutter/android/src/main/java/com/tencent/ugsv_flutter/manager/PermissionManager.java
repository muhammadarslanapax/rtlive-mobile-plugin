...
public class PermissionManager {
        ...
        if (mPermissionType.equals(PermissionType.STORAGE)) {
                ...
                if (Build.VERSION.SDK_INT >= 33) {
                    if (mContext.checkSelfPermission(Manifest.permission.READ_MEDIA_IMAGES) != PackageManager.PERMISSION_GRANTED
                        ...
                        if ((now - oldTime) >= FORTY_EIGHT_HOURS) {
                            checkAndRequestPermission(new String[]{Manifest.permission.READ_MEDIA_IMAGES, Manifest.permission.READ_MEDIA_VIDEO}, REQUEST_CODE_STORAGE);
                    ...
                    if (mContext.checkSelfPermission(PERMISSION_STORAGE) != PackageManager.PERMISSION_GRANTED) {
                        if ((now - oldTime) >= FORTY_EIGHT_HOURS) {
                            checkAndRequestPermission(new String[]{PERMISSION_STORAGE}, REQUEST_CODE_STORAGE);
                            FragmentActivity temp = (FragmentActivity) mContext;
                            mDialog = createDialog();
                            mDialog.show(temp.getSupportFragmentManager(), DIALOG_NAME);
                        }
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                ...
                if (mContext.checkSelfPermission(PERMISSION_AUDIO) != PackageManager.PERMISSION_GRANTED) {
                    if ((now - oldTime) >= FORTY_EIGHT_HOURS) {
                        checkAndRequestPermission(new String[]{PERMISSION_AUDIO}, REQUEST_CODE_AUDIO);
                        FragmentActivity temp = (FragmentActivity) mContext;
                        mDialog = createDialog();
                        mDialog.show(temp.getSupportFragmentManager(), DIALOG_NAME);
                    }
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                ...
                if (mContext.checkSelfPermission(PERMISSION_CAMERA) != PackageManager.PERMISSION_GRANTED) {
                    if ((now - oldTime) >= FORTY_EIGHT_HOURS) {
                        checkAndRequestPermission(new String[]{PERMISSION_CAMERA}, REQUEST_CODE_CAMERA);
                        FragmentActivity temp = (FragmentActivity) mContext;
                        mDialog = createDialog();
                        mDialog.show(temp.getSupportFragmentManager(), DIALOG_NAME);
                    }
    ...
    public boolean checkAndRequestPermission(String permissions[], int requestCode) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
